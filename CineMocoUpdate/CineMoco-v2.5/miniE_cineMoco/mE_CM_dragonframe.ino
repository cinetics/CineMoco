...
uint16_t*          motorAccumulator[MOTOR_COUNT] = { &motorAccumulator0 };
uint16_t           motorMoveSteps0;
uint16_t*          motorMoveSteps[MOTOR_COUNT] = { &motorMoveSteps0 };
uint16_t           motorMoveSpeed0;
uint16_t         * motorMoveSpeed[MOTOR_COUNT] = { &motorMoveSpeed0 };
...
// ====================================================================================
// ====================================================================================
void dragonframe_init(boolean reboot_flag)
{
  
  // wake the motor permanetally up:
  pinMode(30, OUTPUT);
  digitalWrite(30, HIGH);
  
  // save the reboot flag so that 
  // the unit goes to USB mode automatically after
  // the computer connected and the unit rebooted 
  // due to that
  dragonframe_startUSBMode = reboot_flag;
  eeprom_write_config();
  ...
  for (int i = 0; i < MOTOR_COUNT; i++)
  {
    ...
    #if ( PINOUT_VERSION == 2 )
      motors[i].stepPin = 33; //(i * 2) + ( (i < 4) ? 4 : 20 );
    #elif ( PINOUT_VERSION == 1 )
      motors[i].stepPin = (i * 2) + ( (i < 4) ? 4 : 14 );
    #endif
    
    motors[i].dirPin = 35; //motors[i].stepPin + 1;
...
// ===================================================================================
// The dragonfrmae loop
// A comment from the dragonfmrame developers:
// For stepper-motor timing, every clock cycle counts.
// ===================================================================================
void dragonframe_process()
{
